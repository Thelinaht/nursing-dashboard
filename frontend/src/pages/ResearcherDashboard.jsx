...
export default function ResearcherDashboard() {
    ...
    const handlePubFileChange = (e) => {
        ...
        if (!file) return;
        if (file.type !== "application/pdf") { setPubError("Only PDF files are allowed."); return; }
        if (file.size > 10 * 1024 * 1024) { setPubError("File must be 10 MB or smaller."); return; }
        setPubFile(file);
        setPubError("");
    };
    ...
    const generateReport = () => {
        ...
        doc.text("Research Report", pageWidth / 2, y, { align: "center" }); y += 8;
        doc.setFontSize(10); doc.setFont("helvetica", "normal"); doc.setTextColor(100, 116, 139);
        const filterText = yearFilter ? `Year: ${yearFilter}` : "All Years";
        doc.text(`${filterText}  |  Generated: ${new Date().toLocaleDateString("en-GB")}`, pageWidth / 2, y, { align: "center" }); y += 8;
        ...
        const addSectionTable = (label, head, rows) => {
            ...
            doc.text(`${label} (${rows.length})`, margin, y); y += 5;
            if (rows.length === 0) {
                doc.setFontSize(10); doc.setFont("helvetica", "italic"); doc.setTextColor(150);
                doc.text("No entries.", margin + 2, y + 4); y += 12; return;
            }
            autoTable(doc, {
                startY: y, head: [head], body: rows,
                margin: { left: margin, right: margin },
                styles: { fontSize: 9, cellPadding: 4, textColor: [71, 85, 105], lineColor: [203, 213, 225], lineWidth: 0.1 },
        ...
        doc.text("Research Projects", margin, y); y += 8;
        const projectColumns = ["#", "Title", "Investigator", "Start Date", "Status"];
        const buildProjectRows = (list) => list.map((p, i) => [String(i + 1), p.title || "—", p.investigator_name || "—", formatDate(p.start_date), p.status || "—"]);
        ...
        if (y > pageHeight - 40) { doc.addPage(); y = 20; }
        doc.setFontSize(15); doc.setFont("helvetica", "bold"); doc.setTextColor(30, 41, 59);
        doc.text("Publications", margin, y); y += 8;
        const pubColumns = ["#", "Title", "Author", "Type", "Date", "Journal / Conference", "File"];
        const pubRows = filteredPubs.map((p, i) => [String(i + 1), p.title || "—", p.author_name || "—", p.type || "—", formatDate(p.date), p.journal_name || "—", p.PublishedFile_path ? "View PDF" : "—"]);
        if (pubRows.length === 0) {
            addSectionTable("All Publications", pubColumns, pubRows);
        } else {
            if (y > pageHeight - 40) { doc.addPage(); y = 20; }
            doc.setFontSize(13); doc.setFont("helvetica", "bold"); doc.setTextColor(30, 41, 59);
            doc.text(`All Publications (${pubRows.length})`, margin, y); y += 5;
            autoTable(doc, {
                startY: y, head: [pubColumns], body: pubRows,
                margin: { left: margin, right: margin },
                styles: { fontSize: 9, cellPadding: 4, textColor: [71, 85, 105], lineColor: [203, 213, 225], lineWidth: 0.1 },
                headStyles: { fillColor: [51, 65, 85], textColor: [255, 255, 255], fontStyle: 'bold', fontSize: 9 },
                alternateRowStyles: { fillColor: [248, 250, 252] },
                columnStyles: { 6: { textColor: [37, 99, 235], fontStyle: 'bold', halign: 'center' } },
                didDrawCell: (data) => {
                    if (data.section === 'body' && data.column.index === 6) {
                        const pub = filteredPubs[data.row.index];
                        if (pub && pub.PublishedFile_path) {
                            doc.link(data.cell.x, data.cell.y, data.cell.width, data.cell.height, { url: `${BASE_URL}/uploads/${pub.PublishedFile_path}` });
                        }
                    }
                }
            });
                    ...
                    <>
                        ...
                        <div className="rd-card">
                            ...
                            <div className="rd-legend-row">
                                <span className="rd-legend-item"><span className="rd-legend-dot" style={{ background: "#5a6f87" }} />Research Projects</span>
                                <span className="rd-legend-item"><span className="rd-legend-dot" style={{ background: "#9fb3cc" }} />Publications</span>
                                <span className="rd-legend-item"><span className="rd-legend-dot" style={{ background: "#ffffffff" }} />Conference</span>
                            </div>
                            <div className="rd-chart-box">
                                {utilizationData.length === 0 ? (
                                    <div className="rd-chart-empty">No data to display yet. Add research projects or publications to see the chart.</div>
                                ) : (
                                    <ResponsiveContainer width="100%" height="100%">
                                        <BarChart data={utilizationData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }} barGap={0} barCategoryGap="25%">
                        ...
                        <div className="rd-card">
                            <div className="rd-card-header">
                                ...
                                <button onClick={openAddProject} className="rd-primary-btn"><Plus size={16} /> Add Project</button>
                            </div>
                                        ...
                                        <tr className="rd-thead-row">
                                            ...
                                            <th className="rd-th-sticky">
                                                <div className="rd-th-filter">
                                                    ...
                                                    <select value={projectsYearFilter} onChange={(e) => setProjectsYearFilter(e.target.value)} className="rd-col-filter" onClick={(e) => e.stopPropagation()}>
                                    ...
                                    <tbody>
                                        {visibleProjects.length === 0 ? (
                                            <tr><td colSpan={5} className="rd-empty-td">
                                                {projects.length === 0 ? `No research projects yet. Click "Add Project" to get started.` : `No projects in ${projectsYearFilter}. Try a different year.`}
                                            </td></tr>
                                            ...
                                            <tr key={proj.project_id} className="rd-tr">
                                                ...
                                                <td className="rd-td">
                                                    <button onClick={() => openEditProject(proj)} className="rd-icon-btn"><Pencil size={13} /> Edit</button>
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
                        ...
                        <div className="rd-card">
                            <div className="rd-card-header">
                                ...
                                <button onClick={openAddPub} className="rd-primary-btn"><Plus size={16} /> Add Publication</button>
                            </div>
                            <div className="rd-table-scroll">
                                <table className="rd-table">
                                    <thead>
                                        <tr className="rd-thead-row">
                                            ...
                                            <th className="rd-th-sticky">
                                                <div className="rd-th-filter">
                                                    <span>Date</span>
                                                    <select value={pubsYearFilter} onChange={(e) => setPubsYearFilter(e.target.value)} className="rd-col-filter" onClick={(e) => e.stopPropagation()}>
                                                        <option value="all">All</option>
                                    ...
                                    <tbody>
                                        {visiblePublications.length === 0 ? (
                                            <tr><td colSpan={7} className="rd-empty-td">
                                                {publications.length === 0 ? `No publications yet. Click "Add Publication" to get started.` : `No publications in ${pubsYearFilter}. Try a different year.`}
                                            </td></tr>
                                            ...
                                            <tr key={pub.publication_id} className="rd-tr">
                                                ...
                                                <td className="rd-td"><span className="rd-type-badge">{pub.type || "—"}</span></td>
                                                ...
                                                <td className="rd-td">
                                                    {pub.PublishedFile_path ? (
                                                        <a href={`${BASE_URL}/uploads/${pub.PublishedFile_path}`} target="_blank" rel="noopener noreferrer" className="rd-view-link">
                                                            <Eye size={13} /> View PDF
                                                        </a>
                                                    ) : <span className="rd-dash">—</span>}
                                                </td>
                                                <td className="rd-td">
                                                    <button onClick={() => openEditPub(pub)} className="rd-icon-btn"><Pencil size={13} /> Edit</button>
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
                ...
                <div onClick={closePubModal} className="rd-overlay">
                    <div onClick={(e) => e.stopPropagation()} className="rd-modal-box rd-modal-box--wide">
                        ...
                        <div className="rd-fields-col">
                            ...
                            <div>
                                <label className="rd-label">PDF File {editingPubId && pubExistingFile ? "(leave empty to keep current)" : ""}</label>
                                <div className="rd-file-box">
                                    <label htmlFor="pdf-input" className="rd-file-btn"><FileText size={15} /> Choose PDF</label>
                                    <input id="pdf-input" type="file" accept="application/pdf" onChange={handlePubFileChange} className="rd-file-input-hidden" />
                                ...
                                {pubExistingFile && !pubFile && (
                                    <a href={`${BASE_URL}/uploads/${pubExistingFile}`} target="_blank" rel="noopener noreferrer" className="rd-view-link rd-view-link--inline">
