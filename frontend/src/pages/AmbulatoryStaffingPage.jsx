...
export default function AmbulatoryStaffingPage() {
    ...
    const generatePDF = () => {
        const doc = new jsPDF();
        doc.setFontSize(18);
        doc.text("Ambulatory Staffing Report", 14, 22);
        doc.setFontSize(11);
        doc.text(`Generated on: ${new Date().toLocaleString()}`, 14, 30);

        const tableData = filteredAmbulatoryStaffing.map(row => [
            row.unit_name || "N/A",
        ...
        autoTable(doc, {
            startY: 40,
            head: [["Unit Name", "Room Census", "Required Ratio", "Available Nurses", "Total Needed", "Gap", "Status"]],
            body: tableData,
            theme: 'grid',
            headStyles: { fillColor: [74, 106, 133] }
        });

        doc.save("Ambulatory_Staffing.pdf");
    };
        ...
        <Layout logoSrc="/logo.png" role={layoutRole} username={displayUsername}>
            <div className="main" style={{ padding: '0 20px' }}>
                ...
                <div className="no-print" style={{ display: "flex", flexDirection: "column", gap: "12px", marginBottom: "25px" }}>
                    ...
                    <div>
                        <h2 style={{ fontSize: '28px', color: '#2c3e50', margin: 0, fontWeight: 800 }}>Ambulatory Staffing</h2>
                    </div>
                </div>
