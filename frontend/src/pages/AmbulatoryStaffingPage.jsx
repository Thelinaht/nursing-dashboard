...
export default function AmbulatoryStaffingPage() {
  ...
  const filteredAmbulatoryStaffing = ambulatoryStaffing.filter(row => {
    if (ambulatoryFilter === 'All') return true;
    if (ambulatoryFilter === 'ER & Emergency') return row.unit_name.includes('ER') || row.unit_name.includes('Emergency');
    if (ambulatoryFilter === 'OR & Surgery') return (row.unit_name.includes('OR') || row.unit_name.includes('Surgery')) && !row.unit_name.includes('Day');
    if (ambulatoryFilter === 'Dialysis') return row.unit_name.includes('Dialysis') || row.unit_name.includes('Hemodialysis');
    if (ambulatoryFilter === 'Outpatient') return row.unit_name.includes('OPD') || row.unit_name.includes('Clinic');
    if (ambulatoryFilter === 'Day Surgery') return row.unit_name.includes('Day Surgery');
    return true;
  });

  const ambulatoryRowsPerPage = 10;
  const totalAmbulatoryPages = Math.ceil(filteredAmbulatoryStaffing.length / ambulatoryRowsPerPage);
  const currentAmbulatoryPageRows = filteredAmbulatoryStaffing.slice((ambulatoryPage - 1) * ambulatoryRowsPerPage, ambulatoryPage * ambulatoryRowsPerPage);

  const exportAmbulatoryToExcel = () => {
    const csvRows = [];
    csvRows.push(['Unit Name', 'Room Census', 'Required Ratio', 'Available Nurses', 'Total Needed', 'Gap', 'Status'].join(','));
    filteredAmbulatoryStaffing.forEach(row => {
      csvRows.push([
        `"${row.unit_name}"`,
        row.bed_census,
        `"${row.required_ratio}"`,
    ...
    a.setAttribute('download', 'Ambulatory_Staffing.csv');
      ...
      <div className="director-dashboard-container" style={{ padding: '20px 30px' }}>
        ...
        <div className="table-box content-box" style={{ width: '100%' }}>
          <div className="box-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px' }}>
            <h2 className="content-box-title" style={{ margin: 0 }}>Ambulatory Staffing</h2>
            
            <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
              <div className="filter-buttons" style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
                {['All', 'ER & Emergency', 'OR & Surgery', 'Dialysis', 'Outpatient', 'Day Surgery'].map(filter => (
                  <button
                    key={filter}
                    onClick={() => { setAmbulatoryFilter(filter); setAmbulatoryPage(1); }}
                    style={{
                      padding: '6px 12px',
                      fontSize: '12px',
                      borderRadius: '20px',
                      border: 'none',
                      cursor: 'pointer',
                      backgroundColor: ambulatoryFilter === filter ? 'var(--accent-blue)' : '#f1f5f9',
                      color: ambulatoryFilter === filter ? 'white' : '#64748b',
                      fontWeight: ambulatoryFilter === filter ? 600 : 500
                    }}
                  >
                    {filter}
                  </button>
              ...
              <button
                ...
                onClick={exportAmbulatoryToExcel}
              >
                <Download size={14} />
                Export to Excel
              </button>
          ...
          <div className="custom-table" style={{ marginTop: '20px' }}>
            ...
            <div style={{ minHeight: '200px' }}>
              {loading ? (
                <div style={{ textAlign: 'center', padding: '60px', color: '#8ea2b5' }}>
                  Loading Ambulatory staffing data...
                </div>
              ) : currentAmbulatoryPageRows.length > 0 ? currentAmbulatoryPageRows.map((row, idx) => (
            ...
            {totalAmbulatoryPages > 1 && (
              <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '15px', marginTop: '20px', paddingTop: '20px', borderTop: '1px solid #f1f5f9' }}>
                <button 
                  disabled={ambulatoryPage === 1}
                  onClick={() => setAmbulatoryPage(p => Math.max(1, p - 1))}
                  style={{ border: 'none', background: 'none', cursor: ambulatoryPage === 1 ? 'not-allowed' : 'pointer', color: ambulatoryPage === 1 ? '#cbd5e1' : 'var(--accent-blue)', display: 'flex', alignItems: 'center', gap: '5px' }}
                >
                  Previous
                </button>
                <span style={{ fontSize: '12px', color: '#64748b' }}>Page {ambulatoryPage} of {totalAmbulatoryPages}</span>
                <button 
                  disabled={ambulatoryPage === totalAmbulatoryPages}
                  onClick={() => setAmbulatoryPage(p => Math.min(totalAmbulatoryPages, p + 1))}
                  style={{ border: 'none', background: 'none', cursor: ambulatoryPage === totalAmbulatoryPages ? 'not-allowed' : 'pointer', color: ambulatoryPage === totalAmbulatoryPages ? '#cbd5e1' : 'var(--accent-blue)', display: 'flex', alignItems: 'center', gap: '5px' }}
                >
                  Next
                </button>
              </div>
