...
for idx, line in enumerate(jsx_lines):
    ...
    # Replace strings and comments to avoid matching parens inside them
    ...
    for char_idx, char in enumerate(clean_line):
        if char == '(':
            ...
            if stack:
                ...
                print(f"Extra closing parenthesis at {line_num}:{char_idx}: {line.strip()}")

if stack:
    print(f"Unclosed parentheses at end: {stack}")
else:
